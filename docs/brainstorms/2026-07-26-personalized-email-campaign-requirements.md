...
- AE17. **Covers R16, R18.** Given local test-inbox mode with an independently allowlisted destination, explicit confirmation, and a deterministic synthetic fixture of at most 10 records, when the run completes, each completed item has confirmed test-inbox acceptance and delivery outcomes are distinguished from dry-run acceptance. Given a supplied file, absent confirmation, disallowed destination, fixture above 10 records, or unsupported Asynq/test-inbox combination, the invocation fails before campaign work or network activity with zero delivery attempts and no fallback or truncation. Given ambiguous transport acceptance, the item is failed with the indeterminate-delivery reason and is not retried automatically.
- AE18. **Covers R17-R18.** Given Redis and a separate worker process are running and the campaign uses a deterministic generated fixture, when an Asynq dry-run task is retried or executed more than once after enqueue duplication, transient failure, uniqueness-lock expiry, or worker crash, the dry-run effect and terminal lifecycle count are committed idempotently once, while duplicate attempts, retries, and retry exhaustion remain observable. Retry exhaustion classifies the affected eligible item as failed without double counting it. Given an evaluator-supplied recipient file or no separately running worker, the distributed invocation is refused or reaches its documented failure deadline without silently starting an in-process worker.
...
- Optional test delivery cannot expose or contact campaign recipients, cannot exceed its hard send limit, and is not combined with distributed execution in this scope.
...
- One product with independent concepts: The local dry-run CLI remains the authoritative default; guarded delivery and distributed execution remain separately selected, but their composition is deferred until external delivery can be reconciled honestly under at-least-once execution.
